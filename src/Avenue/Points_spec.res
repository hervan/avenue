...
describe("Points.count_grapes_cell", () => {
  let two_green_one_purple_grapes = Cell.Content.Grapes(list{Green, Green, Purple})
  ...
  test("should count all grapes from farm cell", () =>
    expect(
      Points.count_grapes_cell({...empty_cell, content: Farm(A)}, two_green_one_purple_grapes),
    ) |> toEqual(3)
  )
  ...
  test("should count no grapes from empty cell", () =>
    expect(Points.count_grapes_cell(empty_cell, two_green_one_purple_grapes)) |> toEqual(0)
  )
})

describe("Points.count_points", () => {
  let full_grid = Grid.setup(Grid.map_A) |> Array.mapi((i, row) =>
    row |> Array.mapi((j, cell) => {
      ...cell,
      Cell.road: Some((
        j != 0 ? Left : mod(i, 2) == 0 ? Top : Bottom,
        j != 5 ? Right : mod(i, 2) == 0 ? Bottom : Top,
      )),
    })
  )
