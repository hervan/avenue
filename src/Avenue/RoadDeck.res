  ...
  let rec aux = (road_deck, available_cards) => {
    ...
    List.length(road_deck) == grid_columns * grid_rows
      ...
      : {
          ...
          aux(list{Road.Card.card_of_ints(road, color), ...road_deck}, available_cards)
        }
  }
  aux(list{}, [[4, 3], [4, 3], [4, 3], [4, 3], [3, 4], [3, 4]])
}
...
let make = (~road_deck, ~current_card, ~dispatch) => {
  ...
  <g
    ...
    |> List.mapi((i, _) =>
      <rect
        key={i |> string_of_int}
        x={Theme.road_card_thickness *. (i |> float_of_int) |> Js.Float.toString}
        y={Theme.road_card_thickness *. (i |> float_of_int) |> Js.Float.toString}
        width="15"
        height="20"
        rx="2"
        fill="lightblue"
    ...
    | None =>
      <g style={Theme.rotate_card(rotation)}>
        <rect
          x={Theme.road_card_thickness *. (List.length(road_deck) |> float_of_int)
            |> Js.Float.toString}
          y={Theme.road_card_thickness *. (List.length(road_deck) |> float_of_int)
            |> Js.Float.toString}
          width="15"
          height="20"
          rx="2"
    ...
    | Some((road, color)) =>
      <g style={Theme.rotate_card(rotation)}>
        ...
        <g
          transform={"translate(" ++
          ((2.5 +. Theme.road_card_thickness *. (List.length(road_deck) |> float_of_int)
            |> Js.Float.toString) ++
          (" " ++
          ((5.0 +. Theme.road_card_thickness *. (List.length(road_deck) |> float_of_int)
            |> Js.Float.toString) ++
          ")")))}>
          <rect
            width="10"
