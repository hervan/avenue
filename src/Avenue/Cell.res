...
module Content = {
  ...
  let make = (~content) =>
    ...
    | Empty => React.null
    | Grapes(grapes) =>
      grapes |> List.mapi((i, grape) => <Grape key={i |> string_of_int} grape i />) |> arr
...
let make = (~cell, ~dispatch) =>
  <g
    ...
    <rect width="10" height="10" stroke="green" strokeWidth="0.1" fillOpacity="0" />
    <Content content=cell.content />
    {switch cell.road {
    | None => React.null
    | Some(road) => <g strokeWidth="0.5"> <Road road pos=Some((cell.row, cell.col)) /> </g>
    }}
  </g>
