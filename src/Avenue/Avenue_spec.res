...
let minimal_grid_contents = [
  ...
  [Farm(C), Empty, Grapes(list{Green, Green, Green, Purple}), Empty],
  [Empty, Grapes(list{Green, Purple, Purple, Purple}), Empty, Farm(D)],
...
describe("Avenue.advance_stage", () => {
  let base_game =
    Game.load_setup(Some(0), "me", minimal_grid, road_deck, farm_deck) |> Game.begin_game
  let game_yellow = {
    ...base_game.avenue,
    current_card: Some((Road.of_int(0), Yellow)),
  }
  let game_a_0 = game_yellow |> Avenue.advance_stage |> Avenue.discard_top_farm
  ...
  let game_a_4 = game_a_1 |> Avenue.advance_stage |> Avenue.advance_stage |> Avenue.advance_stage
  let game_round_end_a = game_a_4 |> Avenue.advance_stage
  let game_b_0 = game_round_end_a |> Avenue.advance_stage |> Avenue.discard_top_farm
  let game_e_0 =
    game_round_end_a
  ...
  test("should current card be grey", () =>
    expect(game_a_1_grey.current_card) |> toEqual(Some((Road.of_int(0), Road.Card.Grey)))
  )
