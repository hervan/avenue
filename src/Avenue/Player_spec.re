...
describe("Player.recount_points", () => {
  ...
  let previous_round_points_round_b =
    switch (player_round_a.current_round_points) {
    | Some(points) => [points, ...player_round_a.previous_round_points]
  ...
  let player_round_b_recounted =
    player_round_b |> Player.recount_points(farms);

  test("should have correct points for round A", () => {
    expect(player_round_a.current_round_points)
    |> toEqual(Some((Farm.A, 4)))
  });

  test("should have correct points for round B", () => {
    expect((
      player_round_b.current_round_points,
      player_round_b.previous_round_points,
    ))
